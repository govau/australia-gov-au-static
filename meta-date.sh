...
CONTENT_COMMIT_DATE=$(TZ=australia/sydney git log --all -i --grep='content_update' --date=iso --format="%aI" $INDEX_PAGE | head -n1)

# Get last content commit date in readable format
DATE_TEXT=$(TZ=australia/sydney git log --all -i --grep='content_update' --date=format-local:"%l:%M%p %Z %A, %d %B %Y" --format="%ad" $INDEX_PAGE | head -n1)
